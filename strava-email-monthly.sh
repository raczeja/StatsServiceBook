#!/bin/sh
# strava-email-monthly — emails last month's club leaderboard on the 1st of each month.
#
# Cron (added by install.sh):
#   0 8 1 * *  /usr/bin/strava-email-monthly >> /var/log/strava-email-monthly.log 2>&1
#
# SMTP settings and recipient list come from /etc/strava-leaderboard.conf.
# The script is a no-op when email vars are not configured, so it is safe to
# install even before SMTP is set up.
#
# Manual test (send for any arbitrary month without waiting for the 1st):
...
# Resolve required email vars; exit gracefully if not configured.
STRAVA_EMAIL_SMTP="${STRAVA_EMAIL_SMTP:-}"
STRAVA_EMAIL_USER="${STRAVA_EMAIL_USER:-}"
STRAVA_EMAIL_TO="${STRAVA_EMAIL_TO:-}"

if [ -z "$STRAVA_EMAIL_SMTP" ] || [ -z "$STRAVA_EMAIL_USER" ] || [ -z "$STRAVA_EMAIL_TO" ]; then
    log "STRAVA_EMAIL_SMTP / STRAVA_EMAIL_USER / STRAVA_EMAIL_TO not set in $CONFIG — skipping monthly email"
    exit 0
fi

# From: defaults to the address part of STRAVA_EMAIL_USER (strips :app-password suffix).
...
# --- Previous month ----------------------------------------------------------
# Override via env for testing: STRAVA_EMAIL_TEST_MONTH=2026-07 strava-email-monthly
if [ -n "${STRAVA_EMAIL_TEST_MONTH:-}" ]; then
    PREV_MONTH="$STRAVA_EMAIL_TEST_MONTH"
else
    year=$(date +%Y)
    month=$(date +%m)
    if [ "$month" -eq 1 ]; then
    ...
    PREV_MONTH="$(printf '%04d-%02d' "$prev_year" "$prev_month")"
fi
...
MONTH_LABEL="$month_name ${PREV_MONTH%%-*}"

log "building leaderboard email for $PREV_MONTH ($MONTH_LABEL)"
...
# HTML shell: header + embedded CSS (inline-compatible for Gmail / Outlook web).
{
    ...
    printf '<title>Strava Leaderboard - %s</title>' "$MONTH_LABEL"
    printf '<style>'
    ...
    printf '.hd{background:#fc4c02;color:#fff;padding:22px 24px;border-radius:6px 6px 0 0}'
    printf '.hd h1{margin:0 0 4px;font-size:21px;font-weight:700;letter-spacing:-.3px}'
    printf '.hd p{margin:0;font-size:13px;opacity:.82}'
    ...
    printf '<div class="hd"><h1>&#127942; Strava Leaderboard</h1><p>%s</p></div>' "$MONTH_LABEL"
    printf '<div class="cb">'
} > "$BODY"
...
for club_id in $CLUB_IDS; do
    ...
    # Look up the club name from the last generated activities.json.
    club_name=""
    ...
    # Aggregate per-athlete stats for the previous month.
    TABLE="$TMP/table_${club_id}.tsv"
    jq -rn --arg month "$PREV_MONTH" \
    ...
    if [ ! -s "$TABLE" ]; then
        printf '<p class="nd">No activities recorded for %s.</p>' "$MONTH_LABEL" >> "$BODY"
        continue
    fi
    ...
    while IFS="$(printf '\t')" read -r rank name dist time_h time_m elev count; do
        ...
        # Medal emoji for top 3; plain number otherwise.
        case "$rank" in
...
SUBJECT="Strava Leaderboard - $MONTH_LABEL"
log "sending to: $STRAVA_EMAIL_TO"
...
old_IFS="$IFS"; IFS=","
for addr in $STRAVA_EMAIL_TO; do
