...
echo "==> scheduling daily runs: leaderboard '$CRON_TIME', my-activities '$CRON_TIME_ME'"
...
{
  crontab -l 2>/dev/null | grep -v 'strava-leaderboard' | grep -v 'strava-my-activities' || true
  echo "$CRON_LINE"
  echo "$CRON_LINE_ME"
} | crontab -
